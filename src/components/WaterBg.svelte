<script>
  ...
  const waterVertexShader = `uniform sampler2D heightmap;
  ...
  function setup() {
    ...
    function onDocumentMouseMove(event) {
      const distance =
        ((event.clientX - lastMousePos.x) ** 2 +
          (event.clientY - lastMousePos.y) ** 2) **
        0.5;
      if (distance > 1.8) {
